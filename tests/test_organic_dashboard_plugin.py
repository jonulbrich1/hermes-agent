...
T("Hermes chat uses context-capable local model", env.get("HERMES_MODEL") == "gemma4:e2b")
T("Hermes toolsets default to all", env.get("HERMES_TUI_TOOLSETS") == "all")
T("Organic runtime source path resolved", Path(paths["runtime_src"]).parts[-2:] == ("organic_runtime", "src"))
T("MCP command is Python", Path(mcp_config.get("command", "")).name.lower() in {"python", "python.exe"})
...
T("Runtime bridge uses Organic venv", "organic_runtime" in bridge_python.parts and ".venv" in bridge_python.parts)
...
T("Dashboard JS shows processor learning stats", "Processor experiences" in js and "Learned pathways" in js and "Capability frontier" in js)
...
configure_py = (ROOT / "scripts" / "configure_organic_hermes.py").read_text(encoding="utf-8")
ensure_runtime_py = (ROOT / "scripts" / "ensure_organic_runtime.py").read_text(encoding="utf-8")
conversation_loop_py = (ROOT / "agent" / "conversation_loop.py").read_text(encoding="utf-8")
...
T("Hermes conversation loop forwards original user turn", "user_message=original_user_message" in conversation_loop_py)
...
T("Hermes GUI runner starts shared runtime before Chat", "ensure_organic_runtime.py" in run_bat)
...
T("Combined setup enables plugin and configures MCP", "configure_organic_hermes.py" in setup_bat)
T("Linux environment enables fail-closed Organic mode", 'ORGANIC_HERMES_MODE="1"' in env_sh)
T("Linux GUI runner starts Hermes dashboard", "hermes_cli.main dashboard" in run_sh)
T("Linux GUI runner starts shared runtime before Chat", "ensure_organic_runtime.py" in run_sh)
T("Linux setup installs both environments", "organic_runtime/.venv/bin/python" in setup_sh and "/.venv/bin/python" in setup_sh)
T("Cross-platform configurator enables Organic plugin", 'cmd_enable("organic-ai"' in configure_py)
T("Cross-platform configurator registers Organic MCP", '_save_mcp_server("organic-ai"' in configure_py)
T("Cross-platform configurator preserves Qwen semantic mode", '"ORGANIC_SEMANTIC_MODE"' in configure_py and '"pydantic"' in configure_py)
T("Cross-platform configurator preserves Organic backend", '"ORGANIC_BACKEND"' in configure_py and '"mvp"' in configure_py)
T("Cross-platform runtime starter uses POSIX sessions", "start_new_session=os.name != \"nt\"" in ensure_runtime_py)
